    ...
    for column in df.columns:
        if pd.api.types.is_numeric_dtype(df[column]):
            ...
            if outliers.any():
                ...
                logger.warn(f"No outliers detected in column: {column}")
...
def azimuth(mrr):
    ...
    return _azimuth(bbox[0], bbox[1]) if axis1 <= axis2 else _azimuth(bbox[0], bbox[3])
...
def interp1d_windows_single_col(
    ...
    **kwargs,
    ...
    # values = []
    for i in df[df[to_interp].isna()].index:
...
def interp1d_all_lines(
    ...
    if plot is True:
        for n in filled_lines.line.unique():
...
def calculate_misties(
    ...
    for _ind, row in inters.iterrows():
        ...
        assert np.isnan(line_value)
        assert np.isnan(tie_value)
